#' Compute the counterfactual for an auction under the assumption of no resale
#'
...
#' @export
#'
counterfactual_no_resale <- function(
    c, dt, bid_prep_cost, mu_t, sigma_t, lambda_t,
    r_t, k, interps, x_seq, n_seq, f_weights
  ){
  new_lambda <- uniroot(
    function(new_l){
  ...
  return(list('revenue' = sum(dpois(c(1:30), new_lambda) * summ$mean_rev),
              'lambda' = new_lambda))
}
